...
class AdvancedAIPlayer(AIPlayer):
  ...
  def have_player_discard(self, game, epsilon, decay):
    ...
    for discard in range(len(game.players_turn.hand)):
      ...
      if game_copy.player_can_play_card(game_copy.players_turn, game_copy.players_turn.hand[discard]):
        ...
        playable_index_net_result.append((discard, self.get_state_equity(game_copy) - self.estimate_opp_equity(game_copy, monte_carlo_rounds)))
      else:
        game_copy.discard(discard)
        unplayable_index_net_result.append((discard, self.get_state_equity(game_copy) - self.estimate_opp_equity(game_copy, monte_carlo_rounds)))

    if len(unplayable_index_net_result) >= 1:
      return self.make_choice(unplayable_index_net_result, epsilon, decay)
    else:
      return self.make_choice(playable_index_net_result, epsilon, decay)

  def estimate_opp_equity(self, game, monte_carlo_rounds):
    equity = []
    for round in range(monte_carlo_rounds):
      game_copy = copy.deepcopy(game)
      ...
      new_hand.extend(self.get_n_hidden_cards(game_copy, num_hidden_cards))
      new_hand.extend(game_copy.players_turn.visible_hand)
      game_copy.players_turn.hand = new_hand
      equity.append(self.get_state_equity(game_copy))
    return numpy.average(equity)

  def get_n_hidden_cards(self, game, number_hidden_cards):
    cards = []
    possible_cards = []
    possible_cards.extend(game.deck.deck)
    possible_cards.extend(c for c in game.players_turn.hand if c not in game.players_turn.visible_hand)
    random.shuffle(possible_cards)
    for card_index in range(number_hidden_cards):
      cards.append(possible_cards[card_index])
    return cards
  ...
  def get_state_equity(self, game):
    ...
    expected_deadwood_value = self.get_expected_deadwood_value(game)
